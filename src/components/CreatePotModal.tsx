        ...
        <div className="p-6">
          <DialogHeader className="mb-5">
            <div className="flex items-center gap-2">
              {step > 1 && (
                <button onClick={() => setStep(s => s - 1)} className="w-7 h-7 rounded-full flex items-center justify-center text-muted-foreground hover:bg-secondary transition-colors -ml-1">
                  <ChevronLeft size={16} />
                </button>
              )}
              ...
              <span className="ml-auto text-xs text-muted-foreground font-medium">{step}/{totalSteps}</span>
            </div>
          </DialogHeader>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {VISUAL_STYLES.map(({ id, label, Icon, desc }) => (
                    ...
                    >
                      <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${visualStyle === id ? 'bg-primary' : 'bg-secondary'}`}>
                        <Icon size={18} className={visualStyle === id ? 'text-primary-foreground' : 'text-muted-foreground'} />
                      </div>
                      <div>
          ...
          {step === 4 && (
            <div className="space-y-5">
              {/* Toggle card */}
              <div
                className={`rounded-xl border p-4 cursor-pointer transition-all ${requireReceipt ? 'border-primary bg-accent' : 'border-border bg-card'}`}
                onClick={() => setRequireReceipt(r => !r)}
              >
                <div className="flex items-start gap-3">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${requireReceipt ? 'bg-primary' : 'bg-secondary'}`}>
                    <Receipt size={18} className={requireReceipt ? 'text-primary-foreground' : 'text-muted-foreground'} />
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-semibold text-foreground">Require receipt upload</span>
                      <div className={`relative w-10 h-5.5 rounded-full transition-colors flex-shrink-0 ${requireReceipt ? 'bg-primary' : 'bg-muted'}`} style={{ height: '22px', width: '40px' }}>
                        <div className={`absolute top-0.5 w-[18px] h-[18px] bg-white rounded-full shadow transition-all ${requireReceipt ? 'left-[18px]' : 'left-0.5'}`} />
                      </div>
                    </div>
                    <p className="text-xs text-muted-foreground mt-0.5">Members must upload a receipt after each withdrawal</p>
                  </div>
                </div>
              </div>

              {/* Receipt window config */}
              {requireReceipt && (
                <div className="space-y-3 animate-fade-in">
                  <div className="space-y-1.5">
                    ...
                    <p className="text-xs text-muted-foreground">Members have {receiptWindowDays} days to submit a receipt</p>
                  </div>
